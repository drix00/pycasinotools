...
def test_read(filepath_cas):
    ...
    assert results._energy_intensity == pytest.approx(0.0)
    assert results._region_id == 1
    assert results._normalized_energy_intensity == pytest.approx(0.0)
    ...
    assert error is None
    assert results._version == 30105022
    assert results._energy_intensity == pytest.approx(7.268071702406E+05)
    assert results._region_id == 3
    assert results._normalized_energy_intensity == pytest.approx(7.268071702406E-01)
